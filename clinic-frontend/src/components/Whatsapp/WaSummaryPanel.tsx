...
import { SummaryResponse } from '@/types/whatsapp';
import { Sparkles, RefreshCw, Bot } from 'lucide-react';
import { Button } from '@/components/ui/button';

interface WaSummaryPanelProps {
...
const OUTCOME_COLORS: Record<string, string> = {
    ...
    'Booking Amended': 'bg-blue-500/10 border-blue-500/30 text-blue-400',
    'Booking Cancelled': 'bg-red-500/10 border-red-500/30 text-red-400',
    'Enquiry Resolved': 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400',
...
const ENTITY_COLORS: Record<string, string> = {
    hotel_name: 'bg-purple-500/10 border-purple-500/30 text-purple-400',
    booking_date: 'bg-blue-500/10 border-blue-500/30 text-blue-400',
    party_size: 'bg-orange-500/10 border-orange-500/30 text-orange-400',
    guest_name: 'bg-green-500/10 border-green-500/30 text-green-400',
    request_type: 'bg-sky-500/10 border-sky-500/30 text-sky-400',
    email: 'bg-indigo-500/10 border-indigo-500/30 text-indigo-400',
    phone: 'bg-teal-500/10 border-teal-500/30 text-teal-400',
    issue: 'bg-red-500/10 border-red-500/30 text-red-400',
    food_item: 'bg-amber-500/10 border-amber-500/30 text-amber-400',
    room_type: 'bg-violet-500/10 border-violet-500/30 text-violet-400',
};
...
export function WaSummaryPanel({ summary, isLoading, isError, onRefresh }: WaSummaryPanelProps) {
    return (
            ...
            <div className="sticky top-0 z-10 px-6 py-5 bg-card/95 backdrop-blur-md border-b border-border flex items-center justify-between shrink-0">
                ...
                <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-xl bg-secondary flex items-center justify-center border border-border">
                        <Sparkles className="w-4 h-4" style={{ stroke: "url(#sparkle-gradient)" }} />
                    </div>
                    <div>
            ...
            <div className="flex-1 px-6 py-6 space-y-6">
                ...
                {!isLoading && !isError && summary && (
                    <div className="space-y-6 animate-in slide-in-from-bottom-4 fade-in duration-500">
                        {/* Top Cards: Outcome & Sentiment */}
                        <div className="grid grid-cols-2 gap-3">
                            {summary.call_outcome && (
                                <div className="bg-secondary/50 p-3.5 rounded-2xl border border-border shadow-sm hover:border-indigo-500/30 transition-colors group">
                                    ...
                                    <div className={cn(
                                        "min-h-[48px] flex items-center justify-center p-2 rounded-xl border transition-colors",
                            ...
                            {summary.sentiment_meter !== null && summary.sentiment_meter !== undefined && (
                                <div className="bg-secondary/50 p-3.5 rounded-2xl border border-border shadow-sm hover:border-orange-500/30 transition-colors group">
                                    ...
                                    {(() => {
                                        const sentiment = getSentimentInfo(summary.sentiment_meter);
                                        return (
                                            <div className="space-y-2">
                                                <div className="flex items-center justify-between">
                                                    <span className={cn('text-[11px] font-bold', sentiment.bgColor)}>
                                                        {sentiment.label}
                                                    </span>
                                                ...
                                                <Progress
                                                    ...
                                                    indicatorClassName={sentiment.color}
                                                />
                                            </div>
                                        );
                                    })()}
                        ...
                        {summary.summary && summary.summary.length > 0 && (
                            <div className="bg-secondary/50 p-5 rounded-2xl border border-border shadow-sm hover:border-amber-500/30 transition-colors">
                                ...
                                <ul className="space-y-3">
                                    {summary.summary.map((point, i) => (
                                        <li key={i} className="flex gap-2.5 group">
                                            <div className="mt-1 flex items-center justify-center w-4 h-4 rounded-full bg-amber-500/10 text-amber-500 group-hover:bg-amber-500 group-hover:text-black transition-colors shrink-0">
                        ...
                        {summary.key_entities && summary.key_entities.length > 0 && (
                            <div className="bg-secondary/50 p-4 rounded-2xl border border-border shadow-sm hover:border-purple-500/30 transition-colors">
